...
async function getDetail(EmployeeID) {
    try {
        let pool = await sql.connect(config);
        let emp= await pool.request()
        .input('input_parameter', sql.NVarChar, EmployeeID)
        .query("SELECT * from EmployeeDetails where EmployeeID= @input_parameter");
        return emp.recordsets;
    }
    catch (error) {
        console.log(error);
...
async function addDetail(Employee) {
    try {
        ...
        .query("SET ANSI_WARNINGS OFF INSERT INTO EmployeeDetails VALUES(@EmployeeID,@EmployeeName,@PhoneNumber)");
